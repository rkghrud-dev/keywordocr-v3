...
### 확장 키워드 채택 기준
- 기본 상품명 흐름은 유지하고, **확장 키워드는 2~4개만 선별**해서 추가하세요.
- 확장 키워드는 반드시 **원본 상품명 또는 OCR에서 직접 확인되는 항목만** 사용하세요.
- **우선 채택할 확장 키워드**:
  - 구조/작동 방식: `360도 회전`, `오픈형`, `무타공`, `접착식`
  - 규격/호환: `18mm`, `75mm`, `20세트`, `5mm`, `4P`
  - 구성/수량: `10개`, `낱개`, `세트`
  - 설치 방식/사용 공간: `책상`, `욕실`, `싱크대`, `벽면`
- **후순위 채택할 확장 키워드**:
  - 용도/상황/검색 습관 표현: `데스크테리어`, `홈트`, `굿즈`, `쇼윈도`
  - 단, 상품 실체가 흐려지지 않을 때만 사용하세요.
- 추가한 단어가 상품을 더 구체적으로 설명하지 못하면 빼세요.
...
### 표준 표기 정규화
- **상품명은 표준 표기 우선**: `알루미늄`, `스테인리스`, `스탠드`, `스냅버튼`
- OCR 오타나 비표준 표기는 교정하세요: `알미늄`, `스텐드` 등은 그대로 복사하지 마세요.
- 다만 **검색어설정/검색키워드에는 실검색 별칭을 보조적으로 허용**할 수 있습니다. 예: `스테인리스` 상품의 태그에 `스텐` 보조 사용

### 의미 약한 표현 정리
- `작업용`, `보수용`, `설치용`, `정리용`, `교체용`, `소품용` 같은 **추상 용도어를 마지막 칸 채우기용으로 남발하지 마세요.**
- 이런 표현은 앞에 **구체 대상/상황이 있을 때만** 사용하세요. 예: `배관 보수`, `벽면 설치`, `책상 정리`
- `기념`, `정리`, `설치`, `보수`처럼 같은 의미가 반복되면 1회만 남기세요.
...
7. **근거 없는 확장 금지**: OCR이나 원본에 없는 재질, 호환 규격, 대상층, 사용처를 추정해서 넣지 마세요
...
## 최종 검수 체크리스트
- 상품명 앞부분만 읽어도 **무슨 상품인지 즉시 파악**되는지 확인하세요.
- 추가한 확장 키워드마다 **원본 상품명/OCR 근거가 있는지** 확인하세요.
- 같은 의미의 용도어가 2번 이상 반복되지 않았는지 확인하세요.
- 삭제해도 의미가 안 줄어드는 단어는 제거하세요.
- 상품명은 **표준 표기**, 검색어설정/검색키워드는 **실검색 별칭 보완** 원칙을 지키세요.

## ⚠️ 가격 분류 상품의 키워드 변형 규칙 (중요)
같은 원본 상품(GS코드)이 가격대에 따라 2개 이상으로 분리된 경우:
- 상품명 끝에 `(가격대)` 표시가 붙어 있음 (예: `(3,000~5,000원)`, `(8,000~12,000원)`)
- **각 분리 상품마다 키워드를 다르게 조합하세요** — 같은 키워드셋을 그대로 복붙하면 중복 상품으로 잡힐 수 있습니다
- 변형 방법:
  1. **키워드 순서를 다르게 배치** (속성→용도→기능 / 용도→속성→기능 등)
  2. **동의어/별칭으로 일부 치환** (호스클램프→호스밴드, 구리스→그리스 등)
  3. **검색어설정도 순서와 조합을 다르게** — 같은 20개를 순서만 바꾸지 말고, 일부 항목을 다른 표현으로 교체
- 핵심상품명(맨 앞)은 동일하게 유지하되, 뒤따르는 키워드 구성을 변형하세요
...
"""
    ...
    # 확장 skill.md 생성 (PDF 보고서 규칙 반영)
    _generate_keyword_skill_extended_md(export_root, upload_path, date_tag, status_cb)

    # 분할 엑셀 생성 + skill.md 복사
    if chunk_size and chunk_size > 0:
        _split_upload_excel(upload_path, export_root, chunk_size, date_tag, status_cb)
        # llm_chunks 폴더에도 keyword_skill.md + extended 복사 (Codex가 참조할 수 있도록)
        import shutil
        chunks_dir = os.path.join(export_root, "llm_chunks")
        if os.path.isdir(chunks_dir):
            shutil.copy2(md_path, os.path.join(chunks_dir, "keyword_skill.md"))
            ext_md = os.path.join(export_root, "keyword_skill_extended.md")
            if os.path.isfile(ext_md):
                shutil.copy2(ext_md, os.path.join(chunks_dir, "keyword_skill_extended.md"))
            # llm_result 폴더도 llm_chunks 안에 생성
            os.makedirs(os.path.join(chunks_dir, "llm_result"), exist_ok=True)
            os.makedirs(os.path.join(chunks_dir, "llm_result_ext"), exist_ok=True)
            _status(status_cb, f"keyword_skill.md + extended → llm_chunks/ 복사 완료")


def _generate_keyword_skill_extended_md(export_root: str, upload_path: str, date_tag: str, status_cb=None):
    """PDF 보고서 규칙을 반영한 확장 키워드 지시서(keyword_skill_extended.md)를 생성."""
    md_path = os.path.join(export_root, "keyword_skill_extended.md")
    llm_result_dir = os.path.join(export_root, "llm_result_ext")
    os.makedirs(llm_result_dir, exist_ok=True)

    upload_basename = os.path.basename(upload_path)
    result_filename = f"업로드용_{date_tag}_llm.xlsx"
    llm_result_rel = "llm_result_ext"
    result_rel_path = f"{llm_result_rel}/{result_filename}"

    content = f"""# 키워드 생성 지시서 (확장 — 쿠팡·네이버 SEO 최적화)
...
## 핵심 철학 (쿠팡·네이버 공식 가이드 기반)
이 지시서는 쿠팡·네이버 공식 SEO 가이드 분석 결과를 반영합니다.

**기본 원칙:**
- 키워드를 "많이 넣는 것"보다 **정확한 카테고리/상품정보(적합도)** + **불필요한 중복 제거(신뢰도)**가 핵심
- 네이버: 동의어/유의어는 시스템이 자동 처리 → 중복 기재는 **어뷰징 판정** 위험
- 쿠팡: 상품명·카테고리에 이미 있는 단어는 검색어에서 제외, 붙/띄 중복 불필요
- 키워드는 **명사형 중심**으로 압축 — 매칭 안정성이 높음

## 키워드 생성 절차 (이 순서를 반드시 따르세요)

### STEP 1: 카테고리 확인 (적합도의 출발점)
- OCR 텍스트와 원본 상품명을 기반으로 상품의 **정확한 용도/카테고리**를 판단
- 카테고리가 어긋나면 이후 키워드 최적화가 무력화됨
- ❌ 카테고리명 자체(`생활철물`, `운동용품`, `주방용품`)를 상품명에 넣지 마세요

### STEP 2: 핵심 키워드 확정 (상품 유형의 표준명)
- 검색창에 사용자가 실제로 입력하는 **대표 표현 1개**를 선정
- 이것이 상품명/검색어 설계의 뼈대 — 과도한 나열은 지양

### STEP 3: 세부 속성/옵션 수집
- 규격/소재/용량/호환/기능 등 — **롱테일 검색 매칭의 핵심**
- 대표 옵션은 상품명에 포함하면 구체 질의 매칭이 좋아짐

### STEP 4: 브랜드/모델 정규화
- 한글/영문/모델코드 — 정확한 표기 1회만 유지
- 변형(띄어쓰기/한영중복/축약)을 불필요하게 반복하지 않음

### STEP 5: 용도/사용처/의도 키워드 배치
- 이 상품을 어디서/어떻게 쓰는지 (차량, 사무실, 캠핑, 욕실 등)
- `후기/비교/추천/선물` 같은 의도형 문구는 검색어설정에 제한적으로만 사용
- 상품명에 의도형 문구를 과다 삽입하지 마세요

### STEP 6: 최종 정제 (마지막 단계)
- 띄어쓰기 표준화 → 중복 제거 → 금지어 제거 → 길이/개수 제한에 맞춰 컷오프

## 상품명 작성 규칙

### 구조
`[핵심상품명(표준명)] + [대표옵션/규격] + [핵심속성] + [용도/사용처] + [형태/별칭]`

### 상세 규칙
- 공백으로 구분된 키워드 나열, **80~100자** 범위로 최대한 채우기
- **핵심상품명이 반드시 맨 앞** — 뒤에 속성/용도 순으로 배치
- **동의어 활용**: 같은 단어를 반복하지 말고, 두 번째 언급부터는 동의어/별칭 사용
  - 예: `구리스 호스` → 이후에는 `그리스`로, `호스클램프` → 이후에는 `호스밴드`로
...
## 검색어설정 (Cafe24 태그 — 쿠팡 검색어 슬롯 기준)
- **띄어쓰기 없이 붙여쓰기**, 쉼표로 구분, **반드시 20개** 채우세요
- 각 검색어는 2~4음절 단어 조합을 붙여쓰기 (예: `구리스호스`, `고압연장`, `그리스건`)
- **상품명·카테고리에 이미 있는 단어 조합은 제외** — 검색어 슬롯을 새로운 키워드로 채우세요
- 배치 우선순위: 형태/소재/특장점/별칭/영문명/상위분류/용도 중심
- ❌ 상품명과 완전히 동일한 조합을 반복하지 마세요
- ✅ 상품명에서 다 넣지 못한 **용도/상황/별칭/동의어**를 여기에 배치

## 검색키워드 (네이버 쇼핑 태그 기준)
- 공백 구분, 최대 18개
- 총 100자 이내, 읽기 쉬운 표현 우선
- 상품명 + 검색어설정에서 핵심만 추려서 배치
- ❌ 과도한 태그는 적합도에 영향 — 개수를 통제
...
"""
    ...
    _status(status_cb, f"keyword_skill_extended.md 생성: {md_path}")
